...
namespace mm_hook
{
	...
	namespace hook
	{
		...
		template <typename t> void jump(std::uint32_t address, t function, std::uint32_t size)
		{
		...
		template <typename t> auto detour(std::uint32_t source, const t& destination, std::uint32_t size) -> mm_hook::function<t>
		{
